...
- **Backend works locally but crashes/sleeps on Render free tier** — `mediapipe`/`opencv` are memory-heavy. Free tier (512MB RAM) can OOM during video analysis, and the service sleeps after 15 min idle (adds cold-start delay). Consider a paid tier for anything beyond a demo.
