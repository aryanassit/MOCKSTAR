  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1rem' }}>
        <h1 style={{ margin:0, fontSize:22, fontWeight:700, color:'#2E2A25' }}>Interview history</h1>
        <div style={{ display:'flex', alignItems:'center', gap:8, background:'#EFE3D2', border:'1px solid #D8C7B3', borderRadius:10, padding:'6px 12px' }}>
          <span style={{ fontSize:13, color:'#6F6A63' }}>⌕</span>
          <input type="text" placeholder="Search feedback..." value={search} onChange={e=>setSearch(e.target.value)}
            style={{ border:'none', background:'none', outline:'none', fontSize:12, width:140, color:'#2E2A25' }}/>
        </div>
      </div>

      <div style={{ display:'flex', gap:6, marginBottom:14, flexWrap:'wrap' }}>
        {filters.map(({key,label})=>(
          <button key={key} onClick={()=>setFilter(key)} style={{ fontSize:12, padding:'5px 12px', borderRadius:99, cursor:'pointer', fontWeight:500, background:filter===key?'linear-gradient(135deg,#A0AB97,#8F9B88)':'#EFE3D2', color:filter===key?'white':'#6F6A63', border:filter===key?'none':'1px solid #D8C7B3', transition:'all 0.15s' }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {sessions.length===0?(
        <div style={{ textAlign:'center', padding:'4rem 2rem', ...card }}>
          ...
          <p style={{ margin:'0 0 20px', fontSize:14, color:'#6F6A63' }}>Complete your first mock interview to see history here.</p>
          <button onClick={()=>router.push('/upload')} className="btn-green" style={{ padding:'10px 24px', fontSize:14 }}>Start an interview →</button>
        </div>
      ):visible.length===0?(
        <div style={{ textAlign:'center', padding:'3rem', color:'#6F6A63', fontSize:14 }}>No sessions match this filter.</div>
      ):(
        visible.map((s,idx)=>{
          ...
          return(
            <div key={s.id} style={{ ...card, marginBottom:10, overflow:'hidden', animation:`fadein 0.3s ease ${idx*50}ms both` }}>
              <div onClick={()=>setExpanded(open?null:s.id)}
                ...
                onMouseEnter={e=>(e.currentTarget.style.background='rgba(46,42,37,0.02)')}
                onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
                <div style={{ width:34, height:34, borderRadius:9, background:s.overall_score>=75?'rgba(160,171,151,0.2)':'#F3E8DA', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:16, border:'1px solid #D8C7B3' }}>
                  {s.overall_score>=75?'★':'◈'}
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                    <p style={{ margin:0, fontSize:14, fontWeight:600, color:'#2E2A25' }}>Interview #{num}</p>
                    {idx===0&&<span style={{ fontSize:10, background:'rgba(160,171,151,0.2)', color:'#8F9B88', padding:'2px 7px', borderRadius:99, fontWeight:600 }}>Latest</span>}
                  </div>
                  <p style={{ margin:'1px 0 0', fontSize:11, color:'#6F6A63' }}>{ago(s.created_at)} · {qs.length} questions</p>
                </div>
                <div style={{ display:'flex', alignItems:'center', gap:9, flexShrink:0 }}>
                  <div style={{ textAlign:'right' }}>
                    <div style={{ fontSize:17, fontWeight:700, color:sColor(s.overall_score) }}>{s.overall_score}%</div>
                    <div style={{ fontSize:10, color:'#6F6A63' }}>overall</div>
                  </div>
                  <span style={{ color:'#6F6A63', display:'inline-block', transition:'transform 0.2s', transform:open?'rotate(180deg)':'none' }}>▾</span>
                </div>
              </div>
                ...
                <div style={{ padding:'0 15px 14px', animation:'fadein 0.2s ease' }}>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:12 }}>
                    {[{label:'Speech',v:s.speech_score},{label:'Eye contact',v:s.eye_contact_score},{label:'Posture',v:s.posture_score}].map(({label,v})=>(
                      <div key={label} style={{ background:'#F3E8DA', border:'1px solid #D8C7B3', borderRadius:9, padding:'9px', textAlign:'center' }}>
                        <div style={{ fontSize:15, fontWeight:700, color:sColor(v) }}>{v}%</div>
                        <div style={{ fontSize:10, color:'#6F6A63' }}>{label}</div>
                      </div>
                    ))}
                  </div>
                  {qs.length>0&&(
                    <>
                      <p style={{ margin:'0 0 7px', fontSize:11, fontWeight:600, color:'#6F6A63', textTransform:'uppercase', letterSpacing:'0.06em' }}>Questions asked</p>
                      {qs.map((q,i)=>(
                        <div key={i} style={{ padding:'9px 0', borderBottom:i<qs.length-1?'1px solid #EFE3D2':'none', display:'flex', alignItems:'flex-start', gap:9 }}>
                          ...
                          <p style={{ margin:0, fontSize:13, color:'#A0AB97' }}>{q.text}</p>
                        </div>
                      ))}
                    </>
                  )}
                  {s.feedback&&(
                    <div style={{ marginTop:10, padding:'10px 12px', background:'#F3E8DA', border:'1px solid #D8C7B3', borderLeft:'3px solid #A0AB97', borderRadius:9 }}>
                      <p style={{ margin:0, fontSize:12, color:'#6F6A63' }}><strong style={{ color:'#2E2A25' }}>AI feedback:</strong> {s.feedback}</p>
                    </div>
                  )}
