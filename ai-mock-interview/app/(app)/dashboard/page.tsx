...
export default function Dashboard() {
  ...
  return (
    <div>
      ...
      {sessions.length === 0 ? (
        /* Empty state */
        <div className={mounted ? 'anim-scale-in d-200' : ''} style={{ textAlign: 'center', padding: '4rem 2rem', background: '#1e293b', border: '1px solid #334155', borderRadius: '20px', opacity: mounted ? undefined : 0 }}>
          ...
          <button onClick={() => router.push('/upload')} className="btn-glow"
            style={{ background: 'linear-gradient(135deg, #3b82f6, #10b981)', backgroundSize: '200% 200%', color: 'white', border: 'none', borderRadius: '12px', padding: '12px 28px', fontSize: '15px', fontWeight: 700, cursor: 'pointer', boxShadow: '0 6px 20px rgba(59,130,246,0.35)' }}>
            ...
            <div className={`card-hover ${mounted ? 'anim-fade-up d-400' : ''}`} style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '16px', padding: '1.25rem', opacity: mounted ? undefined : 0 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {sessions.slice(0, 3).map((s, i) => (
                  <div key={s.id} className="card-hover" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: '#0f172a', borderRadius: '10px', border: '1px solid #1e293b', cursor: 'pointer', animation: mounted ? `fadeInUp 0.4s cubic-bezier(0.22,1,0.36,1) ${500 + i * 80}ms both` : 'none' }}>
                    <div>
