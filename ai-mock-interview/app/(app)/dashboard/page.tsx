...
function AnimRing({ score, delay=0 }:{ score:number; delay?:number }) {
  ...
  return (
    <div style={{ position:'relative', width:100, height:100 }}>
      <svg width={100} height={100} viewBox="0 0 100 100" style={{ filter:'drop-shadow(0 0 8px rgba(22,163,74,0.4))' }}>
        <circle cx={50} cy={50} r={r} fill="none" stroke="#0d1a0d" strokeWidth={8}/>
      ...
      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
        <span style={{ fontSize:20, fontWeight:700, color:'#f8fafc', lineHeight:1 }}>{d}%</span>
        <span style={{ fontSize:10, color:'#6b8f6b' }}>avg</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom:12 }}>
      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}>
        ...
        <span style={{ fontSize:12, fontWeight:600, color:'#f8fafc' }}>{d}%</span>
      </div>
      <div style={{ background:'#1e3a1e', borderRadius:4, height:6 }}>
...
export default function Dashboard() {
  ...
  if(loading) return(
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'80vh' }}>
      <div style={{ width:36, height:36, border:'3px solid #1e3a1e', borderTopColor:'#16a34a', borderRadius:'50%', animation:'spin 0.8s linear infinite', boxShadow:'0 0 16px rgba(22,163,74,0.3)' }}/>
  ...
  return (
    <div>
      <div className={mounted?'anim-fade-up d-0':''} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1.5rem', opacity:mounted?undefined:0 }}>
        <div>
          <h1 style={{ margin:'0 0 4px', fontSize:22, fontWeight:700, color:'#f8fafc', letterSpacing:'-0.3px' }}>Dashboard</h1>
          <p style={{ margin:0, fontSize:13, color:'#6b8f6b' }}>{sessions.length>0?`${sessions.length} session${sessions.length>1?'s':''} completed`:'No sessions yet — start your first interview'}</p>
        </div>
      ...
      {!hasResume&&(
        <div className={mounted?'anim-fade-up d-100':''} style={{ padding:'11px 14px', background:'rgba(234,179,8,0.08)', border:'1px solid rgba(234,179,8,0.25)', borderRadius:10, marginBottom:14, fontSize:13, color:'#fbbf24', opacity:mounted?undefined:0 }}>
          No resume uploaded. <button onClick={()=>router.push('/upload')} style={{ background:'none', border:'none', color:'#22c55e', cursor:'pointer', fontSize:13, padding:0, fontWeight:600 }}>Upload one →</button>
        </div>
      )}
      ...
      {sessions.length===0?(
        <div className={mounted?'anim-scale-in d-200':''} style={{ textAlign:'center', padding:'4rem 2rem', ...card, opacity:mounted?undefined:0 }}>
          ...
          <p style={{ margin:'0 0 24px', fontSize:14, color:'#6b8f6b', lineHeight:1.6 }}>Complete your first mock interview to see scores here.</p>
        ...
        <>
          <div className={mounted?'anim-fade-up d-100':''} style={{ display:'grid', gridTemplateColumns:'180px 1fr', gap:14, marginBottom:14, opacity:mounted?undefined:0 }}>
            <div className="card-hover" style={{ ...card, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', padding:'1.25rem 1rem' }}>
              ...
              <p style={{ margin:'8px 0 0', fontSize:11, color:'#6b8f6b' }}>Across {sessions.length} sessions</p>
              {sessions.length>1&&<p style={{ margin:'3px 0 0', fontSize:11, color:'#22c55e', fontWeight:600 }}>↑ {sessions[0].overall_score-sessions[sessions.length-1].overall_score} pts since first</p>}
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gridTemplateRows:'1fr 1fr', gap:10 }}>
              {stats.map(({label,v,s})=>(
                <div key={label} className="card-hover" style={{ background:'#050f05', border:'1px solid #1e3a1e', borderRadius:12, padding:'14px 16px' }}>
                  <p style={{ margin:'0 0 4px', fontSize:11, color:'#6b8f6b' }}>{label}</p>
                  <p style={{ margin:0, fontSize:24, fontWeight:700, color:'#f8fafc' }}>{v}{s}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
            <div className={`card-hover ${mounted?'anim-fade-up d-300':''}`} style={{ ...card, padding:'1.1rem 1.25rem', opacity:mounted?undefined:0 }}>
              <p style={{ margin:'0 0 14px', fontSize:14, fontWeight:600, color:'#f8fafc' }}>Skill breakdown</p>
              <SkillBar label="Speech content" value={avgS} color="#22c55e" delay={400}/>
              <SkillBar label="Eye contact"    value={avgE} color="#16a34a" delay={500}/>
            ...
            <div className={`card-hover ${mounted?'anim-fade-up d-400':''}`} style={{ ...card, padding:'1.1rem 1.25rem', opacity:mounted?undefined:0 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
                <p style={{ margin:0, fontSize:14, fontWeight:600, color:'#f8fafc' }}>Recent sessions</p>
                <button onClick={()=>router.push('/history')} style={{ fontSize:12, color:'#22c55e', background:'none', border:'none', cursor:'pointer' }}>View all</button>
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:9 }}>
                {sessions.slice(0,3).map((s,i)=>(
                  <div key={s.id} className="card-hover" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'11px 13px', background:'#050f05', borderRadius:10, border:'1px solid #1e3a1e', cursor:'pointer', animation:mounted?`fadeInUp 0.4s ease ${500+i*70}ms both`:'none' }}>
                    <div>
                      <p style={{ margin:'0 0 1px', fontSize:13, fontWeight:600, color:'#f8fafc' }}>Interview #{sessions.length-i}</p>
                      <p style={{ margin:0, fontSize:11, color:'#6b8f6b' }}>{ago(s.created_at)}</p>
                    </div>
