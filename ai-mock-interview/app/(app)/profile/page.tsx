...
function Toggle({ on, onToggle }:{ on:boolean; onToggle:()=>void }) {
  return (
    <div onClick={onToggle} role="switch" aria-checked={on} tabIndex={0} onKeyDown={e=>e.key==='Enter'&&onToggle()}
      style={{ width:36, height:20, background:on?'#16a34a':'#1e3a1e', borderRadius:10, cursor:'pointer', position:'relative', transition:'background 0.2s', flexShrink:0 }}>
...
export default function Profile() {
  ...
  const card={ background:'#0d1a0d', border:'1px solid #1e3a1e', borderRadius:14, padding:'1.1rem 1.25rem' };

  return (
    <div>
      <h1 style={{ margin:'0 0 1.3rem', fontSize:22, fontWeight:700, color:'#f8fafc' }}>Profile</h1>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>

        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div style={card} className="card-hover">
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
              <div style={{ width:48, height:48, borderRadius:'50%', background:'linear-gradient(135deg,#16a34a,#22c55e)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:17, fontWeight:700, color:'white', flexShrink:0, boxShadow:'0 2px 10px rgba(22,163,74,0.3)' }}>{initials}</div>
              <div>
                {editingName?(
                  <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                    <input autoFocus value={draftName} onChange={e=>setDraftName(e.target.value)}
                      onKeyDown={e=>{ if(e.key==='Enter')saveName(); if(e.key==='Escape')setEditingName(false); }}
                      style={{ border:'none', borderBottom:'1.5px solid #16a34a', background:'none', fontSize:15, fontWeight:600, outline:'none', color:'#f8fafc', padding:'1px 3px', width:140 }}/>
                    ...
                    <button onClick={()=>setEditingName(false)} style={{ fontSize:12, background:'none', border:'none', color:'#4a6f4a', cursor:'pointer' }}>✕</button>
                  </div>
                ):(
                  <div style={{ display:'flex', alignItems:'center', gap:5 }}>
                    <span style={{ fontSize:15, fontWeight:600, color:'#f8fafc' }}>{displayName}</span>
                    <button onClick={()=>{ setEditingName(true); setDraftName(displayName); }} style={{ background:'none', border:'none', cursor:'pointer', color:'#4a6f4a', fontSize:13, transition:'color 0.15s' }}
                      onMouseEnter={e=>(e.currentTarget.style.color='#22c55e')}
                      onMouseLeave={e=>(e.currentTarget.style.color='#4a6f4a')}>✎</button>
                  </div>
                )}
                <p style={{ margin:'2px 0 0', fontSize:12, color:'#6b8f6b' }}>{email}</p>
              </div>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1px', background:'#1e3a1e', borderRadius:9, overflow:'hidden' }}>
              {[{label:'Member since',value:memberSince||'—'},{label:'Plan',value:'Free'},{label:'Interviews',value:String(sessionCount)},{label:'Best skill',value:sessionCount>0?(avgScores.speech>=avgScores.eye&&avgScores.speech>=avgScores.posture?'Speech':avgScores.posture>=avgScores.eye?'Posture':'Eye contact'):'—'}].map(({label,value})=>(
                <div key={label} style={{ background:'#050f05', padding:'9px 11px' }}>
                  <p style={{ margin:0, fontSize:10, color:'#4a6f4a' }}>{label}</p>
                  <p style={{ margin:'2px 0 0', fontSize:13, fontWeight:600, color:'#f8fafc' }}>{value}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={card}>
            <p style={{ margin:'0 0 10px', fontSize:14, fontWeight:600, color:'#f8fafc' }}>Resume on file</p>
            {resumeName?(
              <div style={{ display:'flex', alignItems:'center', gap:9, padding:'9px 11px', background:'rgba(22,163,74,0.08)', border:'1px solid rgba(22,163,74,0.25)', borderRadius:9 }}>
                <span style={{ fontSize:16, flexShrink:0 }}>📄</span>
                <div style={{ flex:1, minWidth:0 }}>
                  <p style={{ margin:0, fontSize:12, fontWeight:600, color:'#22c55e', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{resumeName}</p>
                </div>
              ...
              <div style={{ textAlign:'center', padding:'1.25rem', border:'1px dashed #1e3a1e', borderRadius:9 }}>
                <p style={{ margin:'0 0 10px', fontSize:13, color:'#6b8f6b' }}>No resume uploaded yet</p>
          ...
          <div style={card}>
            <p style={{ margin:'0 0 12px', fontSize:14, fontWeight:600, color:'#f8fafc' }}>Preferences</p>
            {[{label:'Email reminders',sub:'Remind me every 2 days',on:reminders,toggle:()=>setReminders(r=>!r)},{label:'Auto-save recordings',sub:'Keep clips after sessions',on:autoSave,toggle:()=>setAutoSave(s=>!s)}].map(({label,sub,on,toggle})=>(
              <div key={label} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                <div>
                  <p style={{ margin:0, fontSize:13, color:'#f8fafc' }}>{label}</p>
                  <p style={{ margin:0, fontSize:11, color:'#6b8f6b' }}>{sub}</p>
                </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div style={card} className="card-hover">
            <p style={{ margin:'0 0 12px', fontSize:14, fontWeight:600, color:'#f8fafc' }}>Performance summary</p>
            {sessionCount===0?(
              <p style={{ fontSize:13, color:'#6b8f6b', textAlign:'center', padding:'1rem 0' }}>Complete an interview to see your stats.</p>
            ):(
              [{label:'Average score',value:avgScores.overall,color:'#16a34a'},{label:'Avg speech',value:avgScores.speech,color:'#22c55e'},{label:'Avg eye contact',value:avgScores.eye,color:'#4ade80'},{label:'Avg posture',value:avgScores.posture,color:'#86efac'}].map(({label,value,color})=>(
                <div key={label} style={{ marginBottom:11 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                    <span style={{ fontSize:12, color:'#6b8f6b' }}>{label}</span>
                    <span style={{ fontSize:12, fontWeight:600, color:sColor(value) }}>{value}%</span>
                  </div>
                  <div style={{ background:'#1e3a1e', borderRadius:3, height:5 }}>
          ...
          <div style={card}>
            <p style={{ margin:'0 0 10px', fontSize:14, fontWeight:600, color:'#f8fafc' }}>Improvement areas</p>
            {sessionCount===0?(
              <p style={{ fontSize:13, color:'#6b8f6b' }}>No data yet — complete an interview first.</p>
            ):(
              <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:9, padding:'9px 11px', background:'#050f05', border:'1px solid #1e3a1e', borderRadius:9 }}>
                  <span style={{ fontSize:16, flexShrink:0 }}>📋</span>
                  <div>
                    <p style={{ margin:0, fontSize:12, fontWeight:600, color:'#f8fafc' }}>Answer structure</p>
                    <p style={{ margin:0, fontSize:11, color:'#6b8f6b' }}>Use STAR method for behavioral questions</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <div style={{ ...card, borderLeft:'3px solid #ef4444' }}>
            <p style={{ margin:'0 0 8px', fontSize:14, fontWeight:600, color:'#f8fafc' }}>Danger zone</p>
            <p style={{ margin:'0 0 10px', fontSize:12, color:'#6b8f6b' }}>Deletes all sessions, scores, and history permanently.</p>
