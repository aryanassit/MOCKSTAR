...
export default function UploadPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f172a 0%, #172554 100%)', fontFamily: 'system-ui, -apple-system, sans-serif', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>

      {/* Floating orb background */}
      <div className="orb orb-1" style={{ width: '500px', height: '500px', top: '-150px', left: '-100px', background: 'radial-gradient(circle, rgba(59,130,246,0.15) 0%, transparent 70%)' }} />
      ...
      <div className="orb orb-3" style={{ width: '300px', height: '300px', top: '40%', left: '40%', background: 'radial-gradient(circle, rgba(99,102,241,0.1) 0%, transparent 70%)' }} />

      {/* Navbar */}
      <nav className={mounted ? 'anim-fade-up d-0' : ''} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 32px', borderBottom: '1px solid rgba(255,255,255,0.05)', backdropFilter: 'blur(12px)', background: 'rgba(15,23,42,0.7)', position: 'relative', zIndex: 10, opacity: mounted ? undefined : 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '30px', height: '30px', borderRadius: '8px', background: 'linear-gradient(135deg, #3b82f6, #10b981)', backgroundSize: '200% 200%', animation: 'gradientShift 4s ease infinite', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: 'white', fontSize: '14px', boxShadow: '0 4px 12px rgba(59,130,246,0.3)' }}>M</div>
          <span style={{ fontWeight: 700, fontSize: '16px', color: '#f8fafc' }}>Mockstar</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          {['dashboard', 'history'].map(p => (
            <button key={p} onClick={() => router.push(`/${p}`)} style={{ fontSize: '13px', color: '#64748b', background: 'none', border: 'none', cursor: 'pointer', padding: '6px 12px', borderRadius: '8px', textTransform: 'capitalize', transition: 'color 0.15s, background 0.15s' }}
          ...
          <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: 'linear-gradient(135deg, #3b82f6, #10b981)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 700, color: 'white', marginLeft: '4px' }}>
      ...
      {/* Gradient banner */}
      <div className={mounted ? 'anim-fade-up d-100' : ''} style={{ background: 'linear-gradient(135deg, rgba(59,130,246,0.9) 0%, rgba(16,185,129,0.9) 100%)', backdropFilter: 'blur(8px)', padding: '36px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px', position: 'relative', zIndex: 1, opacity: mounted ? undefined : 0 }}>
        <div>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 360px', position: 'relative', zIndex: 1 }}>
        ...
        <div className={mounted ? 'anim-fade-left d-200' : ''} style={{ padding: '36px 32px', borderRight: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', opacity: mounted ? undefined : 0 }}>
          {!isUploaded ? (
            <>
              ...
              <p style={{ margin: '0 0 24px', fontSize: '13px', color: '#64748b' }}>PDF only · Max 5 MB · Processed securely</p>
              ...
              <div
                ...
                style={{
                  flex: 1, border: `2px dashed ${dragOver ? '#3b82f6' : '#334155'}`,
                  ...
                  boxShadow: dragOver ? '0 0 40px rgba(59,130,246,0.2), inset 0 0 40px rgba(59,130,246,0.05)' : 'none',
                }}
              >
                {uploading ? (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
                    <div style={{ position: 'relative', width: '64px', height: '64px' }}>
                      <div style={{ position: 'absolute', inset: 0, border: '4px solid #1e293b', borderTopColor: '#3b82f6', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                      <div style={{ position: 'absolute', inset: '6px', border: '4px solid #1e293b', borderTopColor: '#10b981', borderRadius: '50%', animation: 'spin 1.2s linear infinite reverse' }} />
                    </div>
                    <p style={{ margin: 0, fontSize: '17px', fontWeight: 700, color: '#f8fafc' }}>Uploading securely...</p>
                    <p style={{ margin: 0, fontSize: '13px', color: '#64748b' }}>Processing your resume</p>
                  </div>
                ) : (
                  <>
                    <div style={{
                      width: '72px', height: '72px',
                      background: dragOver ? 'rgba(59,130,246,0.2)' : 'rgba(255,255,255,0.04)',
                      border: `1.5px solid ${dragOver ? 'rgba(59,130,246,0.6)' : '#334155'}`,
                      borderRadius: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center',
                      marginBottom: '20px', fontSize: '30px',
                      transition: 'all 0.25s ease',
                      animation: 'float 4s ease-in-out infinite',
                      boxShadow: dragOver ? '0 0 24px rgba(59,130,246,0.3)' : 'none',
                    }}>📄</div>
                    ...
                    <button className="btn-glow" style={{ background: 'linear-gradient(135deg, #3b82f6, #10b981)', backgroundSize: '200% 200%', color: 'white', border: 'none', padding: '13px 32px', borderRadius: '12px', fontWeight: 700, fontSize: '15px', cursor: 'pointer', boxShadow: '0 6px 20px rgba(59,130,246,0.35)' }}>
                      Select PDF file
                    </button>
                    <p style={{ margin: '18px 0 0', fontSize: '12px', color: '#475569' }}>Drag &amp; drop also supported</p>
                  </>
                )}
            ...
            <div className="anim-scale-in" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', width: '100%', maxWidth: '380px', marginBottom: '32px' }}>
                {[{ label: 'Questions', value: '5' }, { label: 'Est. time', value: '~15 min' }, { label: 'Skills scored', value: '3' }].map(({ label, value }, i) => (
                  <div key={label} className="card-hover anim-pop-in" style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '16px 10px', animationDelay: `${i * 80}ms` }}>
              ...
              <button onClick={() => { setIsUploaded(false); setFileName(''); setMessage(''); }} style={{ marginTop: '12px', background: 'none', border: 'none', color: '#64748b', fontSize: '13px', cursor: 'pointer', transition: 'color 0.15s' }}
        ...
        {/* Right panel */}
        <div className={mounted ? 'anim-fade-right d-300' : ''} style={{ padding: '36px 28px', background: 'rgba(10,22,40,0.6)', backdropFilter: 'blur(8px)', display: 'flex', flexDirection: 'column', gap: '24px', opacity: mounted ? undefined : 0 }}>
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {[
                ...
                <div key={title} className={mounted ? 'anim-fade-up' : ''} style={{ display: 'flex', gap: '12px', animationDelay: `${delay}ms`, animationFillMode: 'both', opacity: mounted ? undefined : 0 }}>
                  <div style={{ width: '36px', height: '36px', background: 'rgba(255,255,255,0.04)', border: '1px solid #334155', borderRadius: '9px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '16px', transition: 'transform 0.2s ease, border-color 0.2s ease' }}
                    onMouseEnter={e => { const el = e.currentTarget; el.style.transform = 'scale(1.1) rotate(-4deg)'; el.style.borderColor = '#3b82f6'; }}
                    onMouseLeave={e => { const el = e.currentTarget; el.style.transform = 'none'; el.style.borderColor = '#334155'; }}>
                    {icon}
                  </div>
                  <div>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '20px' }}>
            ...
            {['Use a well-structured resume with clear project descriptions', 'Make sure your camera and microphone work before starting', 'Find a quiet spot with good lighting facing you'].map((tip, i) => (
              <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', marginBottom: '10px' }}>
                <span style={{ fontSize: '10px', color: '#3b82f6', marginTop: '4px', flexShrink: 0, animation: 'pulse-glow 2s ease infinite', animationDelay: `${i * 300}ms` }}>●</span>
                <p style={{ margin: 0, fontSize: '12px', color: '#94a3b8', lineHeight: 1.5 }}>{tip}</p>
              </div>
            ))}
          </div>

          <div style={{ marginTop: 'auto', padding: '14px', background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.15)', borderRadius: '12px' }}>
            <p style={{ margin: '0 0 4px', fontSize: '12px', fontWeight: 600, color: '#f8fafc' }}>🔒 Your data is safe</p>
            <p style={{ margin: 0, fontSize: '11px', color: '#64748b', lineHeight: 1.5 }}>Your resume is processed securely and never shared with third parties.</p>
          </div>
        </div>
      </div>
    </div>
