...
export default function LandingPage() {
  ...
  return (
    <div style={{ background:'#F3E8DA', color:'#2E2A25', fontFamily:'system-ui,-apple-system,sans-serif', minHeight:'100vh', position:'relative', overflowX:'hidden' }}>
      <style>{`
        ...
        @keyframes pulseDot { 0%,100%{opacity:1;box-shadow:0 0 0 0 rgba(160,171,151,0.5)} 50%{opacity:0.7;box-shadow:0 0 0 6px rgba(160,171,151,0)} }
        ...
        .btn-outline:hover { background:rgba(160,171,151,0.08); border-color:#8F9B88; }
        .feature-pill { display:inline-flex; align-items:center; gap:7px; background:rgba(160,171,151,0.08); border:1px solid rgba(160,171,151,0.2); border-radius:99px; padding:8px 16px; font-size:13px; color:#6F6A63; }
        .card-g { background:#EFE3D2; border:1px solid #D8C7B3; border-radius:16px; transition:transform 0.2s,border-color 0.2s,box-shadow 0.2s; }
        .card-g:hover { transform:translateY(-4px); border-color:#75624E; box-shadow:0 16px 40px rgba(160,171,151,0.12); }
        .step-num { width:36px; height:36px; border-radius:50%; background:linear-gradient(135deg,#A0AB97,#8F9B88); display:flex; align-items:center; justify-content:center; font-size:14px; font-weight:800; color:white; flex-shrink:0; box-shadow:0 4px 12px rgba(160,171,151,0.35); }
        .score-ring { filter:drop-shadow(0 0 10px rgba(160,171,151,0.4)); }
        .visual-card { background:#EFE3D2; border:1px solid #D8C7B3; border-radius:20px; padding:20px; box-shadow:0 24px 60px rgba(0,0,0,0.6); animation:float 5s ease-in-out infinite; }
        .section-label { font-size:11px; font-weight:700; color:#A0AB97; text-transform:uppercase; letter-spacing:0.1em; margin-bottom:10px; }
        .gradient-text { background:linear-gradient(135deg,#8F9B88,#A0AB97,#A0AB97); background-size:200% 200%; -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; animation:gradShift 4s ease infinite; }
      `}</style>
      ...
      {/* Ambient orbs */}
      <div style={{ position:'fixed', width:'600px', height:'600px', top:'-200px', left:'-100px', background:'radial-gradient(circle,rgba(160,171,151,0.08) 0%,transparent 70%)', borderRadius:'50%', pointerEvents:'none', zIndex:0 }} />
      <div style={{ position:'fixed', width:'500px', height:'500px', bottom:'-100px', right:'-100px', background:'radial-gradient(circle,rgba(160,171,151,0.06) 0%,transparent 70%)', borderRadius:'50%', pointerEvents:'none', zIndex:0 }} />
      ...
      <section style={{ maxWidth:'1200px', margin:'0 auto', padding:'80px 48px 60px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'60px', alignItems:'center', position:'relative', zIndex:1 }}>
        ...
        <div className={mounted ? 'anim-left' : ''} style={{ opacity: mounted ? undefined : 0 }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:'7px', background:'rgba(160,171,151,0.1)', border:'1px solid rgba(160,171,151,0.3)', borderRadius:'20px', padding:'6px 14px', marginBottom:'24px' }}>
            <div style={{ width:'7px', height:'7px', borderRadius:'50%', background:'#A0AB97', animation:'pulseDot 2s ease infinite' }} />
            <span style={{ fontSize:'12px', color:'#6F6A63', letterSpacing:'0.3px' }}>AI-powered interview coach</span>
          </div>

          <h1 style={{ fontSize:'clamp(36px,5vw,60px)', fontWeight:800, lineHeight:1.08, letterSpacing:'-2px', margin:'0 0 20px', color:'#2E2A25' }}>
          ...
          <div style={{ display:'flex', gap:'8px', marginBottom:'36px', flexWrap:'wrap' }}>
            {['Structure & Clarity', 'Eye Contact', 'Posture & Presence', 'Speech Quality'].map(p => (
              <span key={p} className="feature-pill">
                <span style={{ width:'6px', height:'6px', borderRadius:'50%', background:'#A0AB97', flexShrink:0 }} />
                {p}
              </span>
            ))}
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'10px', paddingTop:'16px', borderTop:'1px solid rgba(160,171,151,0.1)' }}>
            <div style={{ width:'7px', height:'7px', borderRadius:'50%', background:'#A0AB97', flexShrink:0 }} />
            <div style={{ fontSize:'12px', color:'#6F6A63' }}>Free to start · No credit card required · Works on any resume</div>
          </div>
        </div>

        {/* Right — visual mockup */}
        <div className={mounted ? 'anim-right' : ''} style={{ opacity: mounted ? undefined : 0, position:'relative' }}>
          <div style={{ position:'absolute', top:'-40px', right:'-40px', width:'300px', height:'300px', background:'radial-gradient(circle,rgba(160,171,151,0.15) 0%,transparent 70%)', borderRadius:'50%', pointerEvents:'none' }} />
          ...
          <div className="visual-card">
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'6px', marginBottom:'14px' }}>
              ...
              <div style={{ width:'9px', height:'9px', borderRadius:'50%', background:'#8F9B88' }} />
              <div style={{ flex:1, background:'rgba(46,42,37,0.04)', borderRadius:'5px', padding:'4px 10px', fontSize:'10px', color:'rgba(46,42,37,0.25)', marginLeft:'6px' }}>mockstar.app/interview</div>
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'8px', marginBottom:'14px' }}>
              {[{label:'Speech',v:82,c:'#8F9B88'},{label:'Eye contact',v:71,c:'#A0AB97'},{label:'Posture',v:78,c:'#A0AB97'}].map(({label,v,c})=>(
                <div key={label} style={{ background:'#F3E8DA', border:'1px solid #D8C7B3', borderRadius:'10px', padding:'10px 8px', textAlign:'center' }}>
                  <div style={{ fontSize:'16px', fontWeight:800, color:c }}>{v}%</div>
                  <div style={{ fontSize:'9px', color:'#6F6A63', marginTop:'2px' }}>{label}</div>
                  <div style={{ height:'3px', background:'#D8C7B3', borderRadius:'2px', marginTop:'6px', overflow:'hidden' }}>
            ...
            {/* Recording button */}
            <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'12px 14px', background:'rgba(160,171,151,0.08)', border:'1px solid rgba(160,171,151,0.2)', borderRadius:'12px' }}>
              <div style={{ width:'36px', height:'36px', borderRadius:'50%', background:'linear-gradient(135deg,#A0AB97,#8F9B88)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'15px', boxShadow:'0 0 0 4px rgba(160,171,151,0.2)', animation:'pulseDot 2s ease infinite', flexShrink:0 }}>🎙️</div>
              <div>
                <div style={{ fontSize:'12px', fontWeight:600, color:'#2E2A25' }}>AI is listening...</div>
                <div style={{ fontSize:'10px', color:'#6F6A63' }}>Stop talking when finished</div>
              </div>
              <div style={{ marginLeft:'auto', display:'flex', gap:'2px', alignItems:'flex-end', height:'16px' }}>
                {[4,7,10,7,4,6,9,6,4].map((h,i)=>(
                  <div key={i} style={{ width:'3px', height:`${h}px`, background:'#8F9B88', borderRadius:'2px', opacity:0.7 }} />
                ))}
              </div>
            </div>
          </div>
          ...
          {/* Floating score card */}
          <div style={{ position:'absolute', top:'-20px', right:'-20px', background:'#EFE3D2', border:'1px solid rgba(160,171,151,0.3)', borderRadius:'12px', padding:'12px 16px', boxShadow:'0 8px 24px rgba(0,0,0,0.5)', animation:'float 4s 1s ease-in-out infinite' }}>
            <div style={{ fontSize:'10px', color:'#6F6A63', marginBottom:'4px' }}>Overall score</div>
            <div style={{ fontSize:'24px', fontWeight:800, color:'#8F9B88' }}>78%</div>
            <div style={{ fontSize:'10px', color:'#A0AB97', fontWeight:600 }}>↑ +13 from last session</div>
          </div>

          {/* Floating feedback card */}
          <div style={{ position:'absolute', bottom:'-16px', left:'-24px', background:'#EFE3D2', border:'1px solid rgba(160,171,151,0.2)', borderRadius:'12px', padding:'10px 14px', maxWidth:'200px', boxShadow:'0 8px 24px rgba(0,0,0,0.5)', animation:'float 5s 0.5s ease-in-out infinite' }}>
            <div style={{ fontSize:'10px', color:'#6F6A63', marginBottom:'4px' }}>AI Feedback</div>
            <div style={{ fontSize:'11px', color:'#A0AB97', lineHeight:1.5 }}>Strong technical depth. Work on maintaining eye contact.</div>
          </div>
        </div>
      </section>
      ...
      <section id="features" style={{ maxWidth:'1200px', margin:'0 auto', padding:'80px 48px', position:'relative', zIndex:1 }}>
        <div style={{ textAlign:'center', marginBottom:'56px' }}>
          ...
          <h2 style={{ fontSize:'clamp(28px,4vw,44px)', fontWeight:800, color:'#2E2A25', letterSpacing:'-1.5px', margin:'0 0 14px' }}>Three dimensions.<br />One complete picture.</h2>
          <p style={{ color:'rgba(46,42,37,0.45)', fontSize:'16px', maxWidth:'500px', margin:'0 auto', lineHeight:1.7 }}>Most tools only score what you say. We score how you say it and how you present it.</p>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'20px' }}>
          {[
            { icon:'💬', title:'Speech content', sub:'Answer quality', desc:'Gemini reads your video and evaluates how well your answer covers the question — depth, structure, technical accuracy, and relevance to your actual experience.', color:'#8F9B88' },
            { icon:'👁', title:'Eye contact', sub:'Presence & focus', desc:'Computer Vision tracks whether you are looking at the camera during key moments. Real interviewers notice when you are staring at your own preview instead of them.', color:'#A0AB97' },
            { icon:'🧍', title:'Posture', sub:'Non-verbal confidence', desc:'OpenCV detects whether you are sitting upright, leaning, or collapsing — signals that interviewers read as confidence or nervousness before you say a word.', color:'#A0AB97' },
          ].map(({ icon, title, sub, desc, color }) => (
            <div key={title} className="card-g" style={{ padding:'28px' }}>
              <div style={{ width:'52px', height:'52px', borderRadius:'14px', background:'rgba(160,171,151,0.1)', border:'1px solid rgba(160,171,151,0.2)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'24px', marginBottom:'18px' }}>{icon}</div>
              <div style={{ fontSize:'11px', color, textTransform:'uppercase', letterSpacing:'1px', fontWeight:700, marginBottom:'6px' }}>{sub}</div>
              <div style={{ fontSize:'18px', fontWeight:700, color:'#2E2A25', marginBottom:'12px' }}>{title}</div>
              <div style={{ fontSize:'13px', color:'rgba(46,42,37,0.45)', lineHeight:1.7 }}>{desc}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* How it works */}
      <section id="how-it-works" style={{ background:'rgba(117,98,78,0.4)', borderTop:'1px solid rgba(160,171,151,0.08)', borderBottom:'1px solid rgba(160,171,151,0.08)', position:'relative', zIndex:1 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'20px', position:'relative' }}>
            ...
            {[
              ...
              <div key={n} style={{ textAlign:'center', padding:'24px 16px', position:'relative' }}>
                ...
                <div style={{ fontSize:'15px', fontWeight:700, color:'#2E2A25', marginBottom:'8px' }}>{title}</div>
                <div style={{ fontSize:'12px', color:'rgba(46,42,37,0.45)', lineHeight:1.7 }}>{desc}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'16px' }}>
          {[
            ...
            <div key={title} className="card-g" style={{ padding:'24px' }}>
              ...
              <div style={{ fontSize:'16px', fontWeight:700, color:'#2E2A25', marginBottom:'8px' }}>{title}</div>
              <div style={{ fontSize:'13px', color:'rgba(46,42,37,0.45)', lineHeight:1.7 }}>{desc}</div>
            </div>
          ))}
        </div>
      ...
      <section style={{ position:'relative', zIndex:1, overflow:'hidden' }}>
        <div style={{ maxWidth:'800px', margin:'0 auto', padding:'80px 48px', textAlign:'center' }}>
          <div style={{ background:'rgba(117,98,78,0.8)', border:'1px solid rgba(160,171,151,0.2)', borderRadius:'24px', padding:'60px 48px', position:'relative', overflow:'hidden' }}>
            <div style={{ position:'absolute', inset:0, background:'radial-gradient(circle at 50% 0%,rgba(160,171,151,0.12),transparent 70%)', pointerEvents:'none' }} />
